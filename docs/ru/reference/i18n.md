...
Для интернационализации (к примеру, сообщений об ошибках) вам понадобится **реактивный подход** с [computed()](https://vuejs.org/guide/essentials/computed.html), который автоматически обновляет сообщения об ошибках при смене языка.
